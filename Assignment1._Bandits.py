...
class GreedyAgent(main_agent.Agent):
    def agent_step(self, reward, observation=None):
        ...
        current_action = argmax(self.q_values) # Use the argmax function you created above
        # your code here
...
class EpsilonGreedyAgent(main_agent.Agent):
    def agent_step(self, reward, observation):
        ...
        rndom = np.random.random()
        if rndom < self.epsilon: 
            rndom_action = np.random.randint(0, len(self.q_values))
            current_action = rndom_action
        else: 
