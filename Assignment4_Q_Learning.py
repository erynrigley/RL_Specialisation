...
class QLearningAgent(BaseAgent):
    ...
    def agent_step(self, reward, observation):
        ...
        # --------------------------
        # your code here
        target = max(current_q)
        behaviour = self.q[self.prev_state, self.prev_action]
        self.q[self.prev_state, self.prev_action] += self.step_size * (reward + (self.discount * target) - behaviour)
        # --------------------------
    ...
    def agent_end(self, reward):
        ...
        # your code here
        behaviour = self.q[self.prev_state, self.prev_action]
        self.q[self.prev_state, self.prev_action] += self.step_size * (reward + 0 - behaviour)
...
class ExpectedSarsaAgent(BaseAgent):
    ...
    def agent_step(self, reward, observation):
        ...
        target = 0
        for i in current_q: 
            target += (1 - self.epsilon + (self.epsilon/len(current_q))) * i
        behaviour = self.q[self.prev_state, self.prev_action]
        self.q[self.prev_state, self.prev_action] += self.step_size * (reward + self.discount * target - behaviour)
        # --------------------------
